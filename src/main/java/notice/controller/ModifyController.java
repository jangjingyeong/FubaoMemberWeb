...
public class ModifyController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int noticeNo = Integer.parseInt(request.getParameter("noticeNo"));
		Notice notice = new Notice(noticeNo, noticeSubject, noticeContent); // 생성자를 만드는 방법 
//		notice.setNoticeNo(noticeNo); // 생성자 안만들고 기존 데이터에 추가하는 방법 
		// 쿼리문 UPDATE NOTICE_TBL SET NOTICE_SUBJECT = ?, NOTICE_CONTENT = ? WHERE NOTICE_NO = ?
		NoticeService service = new NoticeService();
		...
		if(result > 0) {
			...
			// 수정 실패 -> 실패 메시지 출력, 공지사항 수정이 완료되지 않았습니다.
